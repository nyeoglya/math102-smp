...
$A_L(D)$를 $D$의 아래쪽 넓이, $A_U(D)$를 $D$의 위쪽 넓이라 한다.
\end{definition}
$([0,1]\times[0,1])\cap (\Q^2)$와 같은 반례의 존재로 인해, 위쪽 넓이와 아래쪽 넓이 값이 항상 같으리라는 보장은 없다. 그러나, 우리는 두 값이 같은 경우를 생각해볼 수 있다.
\begin{definition}[넓이]
$A_L(D)=A_U(D)$이면, 이러한 공통된 값을 D의 넓이라 하며, $\Area(D)=A_L(D)$와 같이 쓴다. 이때, D가 넓이를 갖는다고 한다.
\end{definition}
\begin{definition}[$\R^2$에서의 부드러운 유계]
집합 $D$가 닫혀있고 유계이며 유한한 수의 $C^1$인 $y=f(x)$, $x=f(y)$ 꼴의 함수의 그래프로 경계를 표현할 수 있다고 하자. 그러한 $D$를 부드러운 유계라 한다.
\end{definition}
\begin{theorem}
...
\begin{definition}[부피]
만약 $D\subseteq\R^3$의 위쪽 부피와 아래쪽 부피 값이 동일하면, 그러한 공통된 값을 D의 부피라 하며, $\Volume(D)$와 같이 쓴다. 이때, D가 부피를 갖는다고 한다.
\end{definition}
부드러운 유계도 2차원에서 했던 것과 정확히 동일하게 정의할 수 있다.
\begin{definition}[$\R^3$에서의 부드러운 유계]
집합 $D$가 닫혀있고 유계이며 유한한 수의 $C^1$인 $z=f(x,y)$, $z=f(x,y)$ 꼴의 함수의 그래프로 경계를 표현할 수 있다고 하자. 그러한 $D$를 부드러운 유계라 한다.
\end{definition}
\begin{theorem}
...
위쪽 적분 $I_U(f,D)=\lim_{n\to\infty}U(f,D,2^{-n})$이다. 비슷하게, 아래쪽 적분 $I_L(f,D)=\lim_{n\to\infty}L(f,D,2^{-n})$이다.
\end{definition}
\begin{theorem}
부드럽게 유계인 집합 $D\subseteq\R^2$에서 연속인 비음수 함수 $f$의 위쪽 적분과 아래쪽 적분 값은 동일하다. 
\end{theorem}
\begin{definition}[비음수 함수의 적분가능성]
부드럽게 유계인 집합 $D\subseteq\R^2$에서 정의된 함수 $f$의 위쪽 적분과 아래쪽 적분 값이 동일하면, 우리는 f가 D에서 적분가능하다고 하며, 적분값이 존재한다고 한다. 이때, 함수 f의 D에서의 적분 값을 아래와 같이 쓴다.
$$
...
$$
를 정의할 수 있다. 이러한 합을 근사 적분, 혹은 리만 합이라 한다.
\end{definition}
\begin{theorem}
...
유계인 집합 D가 함수 $x=a(y)$, $x=b(y)$에 대해 $a(y)\le x\le b(y)$, $c\le y\le d$ 형태로 표현되면 x-단순이라고 한다. 비슷하게, $y=a(x)$, $y=b(b)$에 대해 $c\le x\le d$, $a(x)\le y\le b(x)$ 형태로 표현되면 y-단순이라고 한다.
\end{definition}
...
$U\subseteq\R^2$에서 $C^1$인 함수 $F(u,v)=(x(u,v),y(u,v))$가 일대일 함수이고, F의 미분행렬이 U의 각 점에서 역행렬이 존재하면 F를 부드러운 변수치환이라고 한다.
\end{definition}
...
닫힌 집합 $D\subseteq\R^n$가 부드럽게 유계라는 것은 $D$의 경계가 아래와 같은 형태의 유한한 $C^1$ 함수의 그래프의 합칩합으로 표현이 된다는 것이다.
$$
...
$f: \R^n \to \R$가 부드럽게 유계인 집합 $D\subseteq\R^n$에서 유계라고 하자. 위쪽/아래쪽 적분은 아래와 같이 정의된다.
$$???$$
만약, 위쪽 적분값과 아래쪽 적분값이 동일하면, $f$가 $D$에서 적분가능하다고 하며, 아래와 같이 쓴다.
$$
...
부드럽게 유계인 $D\subseteq\R^n$에서 적분가능한 함수 $f$의 평균은 다음과 같이 정의된다.
$$
