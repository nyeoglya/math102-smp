...
\end{align*}
$(A,B)=(a_1,\cdots,a_n,b_1,\cdots,b_m) \in S$이고, 편도함수 행렬 $[\frac{\partial f_i}{\partial y_j}(A,B)]$의 행렬식이 0이 아니라고 하자. 그러면, $C^1$ 함수 $G: \R^n \to \R^m$가 존재하여, (A,B)와 충분히 가까운 점 $(X,Y) \in S$에 대해, $(X,Y)=(X,G(X))$인 경우, 아래를 만족한다.
\begin{align*}
